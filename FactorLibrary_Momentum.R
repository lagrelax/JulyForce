...
log_rtn <- log_price %>% group_by(ticker) %>% arrange(rev(date)) %>% mutate(log1R=c(-diff(logP,1),NA))
...
for(loop_idx in start_idx:length(biz_dates))
{
  ...
  for(name in filtered_tickers)
  {
    print(paste('Calcuating ticker',name))
    ticker_rtn_df <- rtn_df %>% filter(ticker==name)
    ema <- EMA(ticker_rtn_df[,'log1R'],ratio=decay)
    momentum <- ema[length(ema)]
    result <- rbind(result,data.frame(ticker=name,date=loop_date,Momentum=momentum))
  }
}
